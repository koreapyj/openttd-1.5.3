...
struct TimetableWindow : Window {
	...
	virtual void OnPaint()
	{
		...
		this->SetWidgetDisabledState(WID_VT_START_DATE, HasBit(v->vehicle_flags, VF_AUTOMATE_TIMETABLE));
		this->SetWidgetDisabledState(WID_VT_RESET_LATENESS, HasBit(v->vehicle_flags, VF_AUTOMATE_TIMETABLE));
		...
		switch (widget) {
			...
			case WID_VT_START_DATE: // Change the date that the timetable starts.
				if (_settings_client.gui.time_in_minutes && _settings_client.gui.timetable_start_text_entry) {
					...
					time /= _settings_game.economy.ticks_per_minute;
					time += _settings_game.economy.clock_offset;
					time %= 24 * 60;
		...
		switch (this->clicked_widget) {
			...
			case WID_VT_START_DATE: {
				...
				if (val > 0) {
					...
					val -= _settings_game.economy.clock_offset;
